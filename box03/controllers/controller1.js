...
var model2 = mongoose.model('rez2_collection2', mongoose.Schema({item: String}) );

//var nkb = new Schema({name: String, keyword: String, body: String});
		...
		var data = model3.find({}, function(err, data){
			...
			function treeMe(myObj){
				...
				for (x in myObj){
					...
					var y;
					for (y = 0; y <= myObj[x].texts.text.length; y++){
						if( myObj[x].texts.text[y] ){ text += "<p>" + myObj[x].texts.text[y] +"</p>";}
					}
					//treeMe(myObj[x].title);
